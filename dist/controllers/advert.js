    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            ORDER BY
                mc.category_id ASC
        `;
            const categoryResults = yield pool.query(categorySqlQuery);
...
exports.getActualAdvert = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            let sqlNonAuthQuery;
            if (currentUser === null || currentUser === void 0 ? void 0 : currentUser.user_id) {
                sqlNonAuthQuery = `
                 SELECT 
                    ad.id, 
                    ad.title, 
            ...
            const data = yield pool.query(sqlNonAuthQuery, values);
            let result = data.rows;
            if (result && (currentUser === null || currentUser === void 0 ? void 0 : currentUser.user_id)) {
                const favoriteQuery = `
                SELECT 
                    ad.id
                FROM 
                ...
                WHERE 
                    ...
                    adf.user_id = $1
                `;
                const favoriteResult = yield pool.query(favoriteQuery, [currentUser === null || currentUser === void 0 ? void 0 : currentUser.user_id]);
                const favoriteData = favoriteResult.rows;
                const newData = result.map((item) => {
                    const hasId = favoriteData.find((idObj) => idObj.id == item.id);
                    return Object.assign(Object.assign({}, item), { has_favorite: hasId ? true : false });
                });
                result = newData;
            }
...
exports.getAdvertDetail = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const data = yield pool.query(sqlQuery, [advert_id]);
            ...
            WHERE
                advert_id = $1
        `;
            const imagesResponse = yield pool.query(imagesQuery, [advert_id]);
...
exports.getMyAdvert = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const getRedisData = yield redis.RedisClient.get('currentUser');
        const currentUser = JSON.parse(getRedisData);
        const userId = currentUser === null || currentUser === void 0 ? void 0 : currentUser.user_id;
        try {
            const sqlQuery = `
            SELECT
                ad.id,
            ...
            GROUP BY
				ad.id
        `;
            const response = yield pool.query(sqlQuery, [userId]);
...
exports.postAdvert = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const getRedisData = yield redis.RedisClient.get('currentUser');
        ...
        try {
            ...
            const status = yield pool.query(insertQuery, values);
            const advert_id = status.rows[0].id;
            const advertImagesResponse = yield Image.uploadMultipleImages(advertImages, 'members/' + email + '/adverts/' + advert_id + '/', title);
            ...
            `;
                ...
                yield pool.query(imageInsertQuery, values);
            }
...
exports.getMyAdvertDetail = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const getRedisData = yield redis.RedisClient.get('currentUser');
        const currentUser = JSON.parse(getRedisData);
        const userId = currentUser === null || currentUser === void 0 ? void 0 : currentUser.user_id;
        ...
        try {
            ...
            AND
                ad.user_id = $2
        `;
            const advertHasResult = yield pool.query(advertHasSqlQuery, [advert_id, userId]);
        ...
        `;
            const data = yield pool.query(advertSqlQuery, [advert_id]);
            const advertDetail = data.rows[0];
            return res.status(200).json(advertDetail);
        }
        catch (err) {
...
exports.putAdvertEdit = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const getRedisData = yield redis.RedisClient.get('currentUser');
        const parseUser = JSON.parse(getRedisData);
        const user_id = parseUser.user_id;
        ...
        try {
            ...
            AND
                ad.user_id = $2
        `;
            const selectResponse = yield pool.query(selectQuery, [advertId, user_id]);
            const selectResult = selectResponse.rows[0];
            if (!selectResult) {
                throw new CustomError(204, "bulunamadı");
            }
            const advertImagesResponse = yield Image.uploadMultipleImages(advertImages, 'members/' + email + '/adverts/' + advertId + '/', title);
            for (const item of advertImagesResponse) {
                const imageInsertQuery = `
                INSERT INTO advert_images
                    (url, path, width, height, advert_id, is_cover_image) 
            ...
            const deletedOldImage = (imageId, path) => __awaiter(this, void 0, void 0, function* () {
                yield Image.removeOldImage(path);
                ...
                WHERE
                    images_id = $1
            `;
                yield pool.query(imageDeleteQuery, [imageId]);
            });
        ...
        `;
            if (coverImageId !== 'undefined') {
                ...
                yield pool.query(coverImageQuery, updateValues);
            }
...
exports.patchSettingAdvert = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const advert_id = req.params.advert_id;
        ...
        try {
            if (path == "has_advert_visible" && op == 'replace') {
                ...
                WHERE
                    id = $2
            `;
                yield pool.query(visibleQuery, [value, advert_id]);
                return res.status(200).json({ 'success': true });
            }
...
exports.getLocationCity = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const data = yield pool.query(`
            SELECT 
                * 
            FROM 
                cities
...
exports.getCountyForCity = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            WHERE
                city_id = $1
        `;
            const data = yield pool.query(sqlQuery, [city_id]);
            const response = data.rows;
            return res.status(200).json(response);
        }
        catch (err) {
...
exports.patchFavoriteAdvert = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const getRedisData = yield redis.RedisClient.get('currentUser');
        const currentUser = JSON.parse(getRedisData);
        const advertId = req.params.advert_id;
        ...
        try {
            ...
            AND
                advert_id = $2
        `;
            const responseData = yield pool.query(sqlQuery, [userId, advertId]);
            ...
            if (result && op == 'remove') {
                ...
                WHERE
                    favorite_id = $1
            `;
                yield pool.query(deleteQuery, [favoriteId]);
                ...
                RETURNING *
            `;
                yield pool.query(insertQuery, [userId, advertId]);
...
exports.getMyFavoriteAdvert = function (req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        const getRedisData = yield redis.RedisClient.get('currentUser');
        const currentUser = JSON.parse(getRedisData);
        const userId = currentUser === null || currentUser === void 0 ? void 0 : currentUser.user_id;
        try {
            ...
            WHERE 
                ...
                adf.user_id = $1
        `;
            const responseData = yield pool.query(sqlQuery, [userId]);
